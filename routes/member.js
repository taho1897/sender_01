...
router.get('/me', isSecure, isAuthenticated, function(req, res, next) {
    res.send({
        result: {
            ...
            pic : 'http://ec2-52-78-70-38.ap-northeast-2.compute.amazonaws.com:80/images/upload_20c413748c8b88ae38a10c03370cc850.jpg',
            activation : 1 // 활성화 유무
        }
    });
}); // 3. 자신의 정보 보기
...
router.get('/:user_id', isSecure, isAuthenticated, function(req, res, next) {
    ...
    res.send({
        result : {
            ...
            deliver_com: 3,
            deliver_req: 1,
            pic: 'http://ec2-52-78-70-38.ap-northeast-2.compute.amazonaws.com:80/images/upload_01bd18c7dd4fa45013be85aef8ed10a5.jpg',
            activation: 1
        }
    });
}); // 4. 특정 사용자의 정보 보기
...
router.put('/me', isAuthenticated, function(req, res, next) {
    ...
    form.parse(req, function(err, fields, files) {
        ...
        menu.files = [];
            ...
            menu.files.push({url : url.resolve('http://ec2-52-78-70-38.ap-northeast-2.compute.amazonaws.com:3000','/images/'+filename)});
